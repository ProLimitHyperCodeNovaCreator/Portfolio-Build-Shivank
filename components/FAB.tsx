  ...
  return (
    ...
    `}
        >
          {/* Background fill */}
          <div
            className={`absolute inset-0 z-0 bg-gradient-to-b from-red-600/20 to-red-800/30 transition-all duration-500 transform ${
              isExpanded ? "scale-y-100 origin-top" : "scale-y-0 origin-top"
            }`}
          ></div>
          ...
          >
            ...
            {/* Sliding text */}
            <span
              className={`relative z-10 ml-0 w-0 overflow-hidden transition-all duration-500 ease-in-out ${
                isExpanded ? "ml-2 w-20" : ""
              }`}
            >
            ...
            <div
              className={`${
                isExpanded
                  ...
                  : "pointer-events-none p-0 space-y-0"
              }`}
            >
              {menuButtons.map((button, index) => (
              <button
                key={index}
                className="group relative w-full flex items-center gap-3 text-white px-4 py-3 rounded-xl text-sm font-medium cursor-pointer transition-all duration-300 hover:scale-[1.02] backdrop-blur-sm border border-white/10 hover:border-red-400/50 overflow-hidden"
      ...
      <div className="hidden md:block">
        ...
        >
          ...
          <span
            className={`absolute inset-0 z-0 bg-red-500 transition-all duration-500 transform ${
              isExpanded ? "scale-x-100 origin-left" : "scale-x-0 origin-left"
            }`}
          ></span>

          {/* Menu icon - always visible */}
          <div
            className={`absolute right-0 top-0 h-full flex items-center justify-center w-28 z-20 ${
          ...
          <div
            className={`relative flex gap-1.5 items-center pl-4 h-full z-10 transition-all duration-500 ease-in-out overflow-hidden ${
